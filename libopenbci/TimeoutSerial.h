 ...
 * necessary subset of the functions provided by the OpenFramework's ofSerial
 * class so that it can serve as a drop in replacement to a templated driver library
 * for communicating with openBCI projects
 * 
 * The implementation currently relies on the boost library, although future versions may switch this
 * dependency to POCO.
 * 
 * Forked from https://gitorious.org/serial-port/serial-port/
